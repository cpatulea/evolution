...
% ----------------------------------------------------------------------------
NVidia's Compute Unified Device Architecture (CUDA) is a programming paradigm for massively parallel processors present in off-the-shelf graphics cards (GPUs). GPUs consist of many independent floating-point computation units, providing significant speedup to data-parallel compute-intensive applications. Each unit is connected to on-board memory using a very wide bus, enabling very high memory bandwidth provided certain particular memory access patterns are respected. The combination of these features makes CUDA an ideal platform for GA-ANN training.
...
Item 1 is not very computationally intensive and was not parallelized. Items 2, 3 and 4 were all implemented on the CUDA platform. Performance results are given for the combined execution time of these three items.
